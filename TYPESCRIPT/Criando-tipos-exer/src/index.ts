...
function savePlanet(
  ...
  situation: Situation = "inexplorado",
  satelites: string[] = []
) {
  const planet = { name, coordenates, situation, satelites };
  const confirmation = confirm(
    `Deseja salvar o planeta de nome ${planet.name}?`
  );
  if (confirmation) {
    planets.push(planet);
  }
...
function changeSituation(planet: { name: string; situation: Situation }) {
  const newSituation = prompt(
    `Para qual situação deseja mudar o planeta ${planet.name}?\n "habitado" | "habitável" | "inabitável" | "inexplorado"`
  );
  if (
    newSituation === "habitado" ||
    newSituation === "habitável" ||
    newSituation === "inabitável" ||
    newSituation === "inexplorado"
  ) {
    planet.situation = newSituation as Situation;
...
function addSatelite(planet: { name: string; satelites: string[] }) {
  const newSatelite = prompt(
    `Qual o nome do satélite que deseja adicionar ao planeta ${planet.name}?`
  );

  const confirmation = confirm(
    `Deseja mesmo adicionar o satélite de nome ${newSatelite} ao planeta ${planet.name}?`
  );
...
function removeSatelite(planet: { name: string; satelites: string[] }) {
  const newSatelite = prompt(
    `Qual o nome do satélite que deseja remover do planeta ${planet.name}?`
  );

  const confirmation = confirm(
    `Deseja mesmo remover o satélite de nome ${newSatelite} do planeta ${planet.name}?`
  );
...
function firstOption() {
  ...
  const confirmation = confirm(
    `Deseja salvar o planeta ${name} com as coodernadas ${coordenates1}, ${coordenates2}, ${coordenates3}, ${coordenates4} `
  );
...
function thirdOption() {
  const name = prompt(
    "Qual o nome do planeta que deseja adicionar um satélite?"
  );
  const planet = findPlanet(name);
